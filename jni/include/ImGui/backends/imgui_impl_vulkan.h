...
IMGUI_IMPL_API void             ImGui_ImplVulkan_Shutdown(bool destroy_device_objects = true);
IMGUI_IMPL_API void             ImGui_ImplVulkan_NewFrame();
IMGUI_IMPL_API void             ImGui_ImplVulkan_RenderDrawData(ImDrawData* draw_data, VkCommandBuffer command_buffer, VkPipeline pipeline = VK_NULL_HANDLE);
IMGUI_IMPL_API void             ImGui_ImplVulkan_SetMinImageCount(uint32_t min_image_count); // To override MinImageCount after initialization (e.g. if swap chain is recreated)
IMGUI_IMPL_API bool             ImGui_ImplVulkan_SetImageCount(uint32_t image_count); // Caller must synchronize the queue before changing this value.
IMGUI_IMPL_API bool             ImGui_ImplVulkan_SetImageCounts(uint32_t min_image_count, uint32_t image_count); // Caller must synchronize the queue before changing these values.
...
// Helpers
IMGUI_IMPL_API bool                 ImGui_ImplVulkanH_CreateOrResizeWindow(VkInstance instance, VkPhysicalDevice physical_device, VkDevice device, VkQueue queue, ImGui_ImplVulkanH_Window* wd, uint32_t queue_family, const VkAllocationCallbacks* allocator, int w, int h, uint32_t min_image_count, VkImageUsageFlags image_usage, uint64_t wait_timeout_ns);
IMGUI_IMPL_API bool                 ImGui_ImplVulkanH_WaitForQueue(VkDevice device, VkQueue queue, ImGui_ImplVulkanH_Window* wd, const VkAllocationCallbacks* allocator, uint64_t wait_timeout_ns);
