...
bool ImGui_ImplVulkanH_CreateWindowSwapChain(VkPhysicalDevice physical_device, VkDevice device, VkQueue queue, ImGui_ImplVulkanH_Window* wd, const VkAllocationCallbacks* allocator, int w, int h, uint32_t min_image_count, VkImageUsageFlags image_usage, uint64_t wait_timeout_ns);
bool ImGui_ImplVulkanH_CreateWindowCommandBuffers(VkPhysicalDevice physical_device, VkDevice device, ImGui_ImplVulkanH_Window* wd, uint32_t queue_family, const VkAllocationCallbacks* allocator);

static constexpr uint64_t IMGUI_IMPL_VULKAN_FENCE_WAIT_TIMEOUT_NS = 2ULL * 1000ULL * 1000ULL * 1000ULL;
...
void ImGui_ImplVulkan_RenderDrawData(ImDrawData* draw_data, VkCommandBuffer command_buffer, VkPipeline pipeline)
{
    ...
    ImGui_ImplVulkan_Data* bd = ImGui_ImplVulkan_GetBackendData();
    if (bd == nullptr || bd->TextureUploadFailed)
        return;
    ImGui_ImplVulkan_InitInfo* v = &bd->VulkanInitInfo;
...
void ImGui_ImplVulkan_UpdateTexture(ImTextureData* tex)
{
    ...
    if (tex->Status == ImTextureStatus_WantDestroy && tex->UnusedFrames >= (int)v->ImageCount)
    {
        ImGui_ImplVulkan_DestroyTexture(tex);
        return;
    }
    if (bd->TextureUploadFailed)
        return;
    ...
    if (tex->Status == ImTextureStatus_WantCreate)
    {
        ...
        auto fail_create = [&]()
        {
            ...
            if (backend_tex->ImageView != VK_NULL_HANDLE)
                vkDestroyImageView(v->Device, backend_tex->ImageView, v->Allocator);
            if (backend_tex->Image != VK_NULL_HANDLE)
                vkDestroyImage(v->Device, backend_tex->Image, v->Allocator);
            if (backend_tex->Memory != VK_NULL_HANDLE)
                vkFreeMemory(v->Device, backend_tex->Memory, v->Allocator);
            IM_DELETE(backend_tex);
            bd->TextureUploadFailed = true;
        };
    ...
    if (tex->Status == ImTextureStatus_WantCreate || tex->Status == ImTextureStatus_WantUpdates)
    {
        ...
        auto fail_upload = [&](VkResult result, bool resources_may_be_in_use)
        {
            check_vk_result(result);
            bd->TextureUploadFailed = true;
            if (!resources_may_be_in_use)
                cleanup_upload();
        };
        if (upload_w <= 0 || upload_h <= 0 || tex->BytesPerPixel <= 0 || upload_size == 0)
        {
            fail_upload(VK_ERROR_FORMAT_NOT_SUPPORTED, false);
            return;
        }
        {
            VkBufferCreateInfo buffer_info = {};
            ...
            if (alloc_info.memoryTypeIndex == 0xFFFFFFFF)
            {
                fail_upload(VK_ERROR_FEATURE_NOT_PRESENT, false);
                return;
            }
        ...
        // Upload to Buffer:
        {
            ...
            {
                fail_upload(err != VK_SUCCESS ? err : VK_ERROR_MEMORY_MAP_FAILED, false);
                return;
            }
        ...
        // End command buffer
        {
            ...
            err = vkResetFences(v->Device, 1, &bd->TexFence);
            check_vk_result(err);
            if (err != VK_SUCCESS)
            {
                fail_upload(err, false);
            ...
            err = vkQueueSubmit(v->Queue, 1, &end_info, bd->TexFence);
            check_vk_result(err);
            if (err != VK_SUCCESS)
            {
                fail_upload(err, false);
        ...
        err = vkWaitForFences(v->Device, 1, &bd->TexFence, VK_TRUE, IMGUI_IMPL_VULKAN_FENCE_WAIT_TIMEOUT_NS);
        check_vk_result(err);
        if (err != VK_SUCCESS)
        {
            fail_upload(err, true);
            return;
        }
        vkDestroyBuffer(v->Device, upload_buffer, v->Allocator);
        vkFreeMemory(v->Device, upload_buffer_memory, v->Allocator);
...
static bool ImGui_ImplVulkan_CreateShaderModules(VkDevice device, const VkAllocationCallbacks* allocator)
{
    ImGui_ImplVulkan_Data* bd = ImGui_ImplVulkan_GetBackendData();
    ImGui_ImplVulkan_InitInfo* v = &bd->VulkanInitInfo;
...
{
    ImGui_ImplVulkan_Data* bd = ImGui_ImplVulkan_GetBackendData();
    if (!ImGui_ImplVulkan_CreateShaderModules(device, allocator))
    ...
    check_vk_result(err);
    return err == VK_SUCCESS ? pipeline : VK_NULL_HANDLE;
}
...
bool ImGui_ImplVulkan_CreateDeviceObjects()
{
    ...
    if (!bd->TexFence)
    {
        ...
        err = vkCreateFence(v->Device, &info, v->Allocator, &bd->TexFence);
        check_vk_result(err);
        if (err != VK_SUCCESS)
            return false;
    }
...
void    ImGui_ImplVulkan_DestroyDeviceObjects()
{
    ...
    if (bd->TexFence)             { vkDestroyFence(v->Device, bd->TexFence, v->Allocator); bd->TexFence = VK_NULL_HANDLE; }
...
{
    if (!g_FunctionsLoaded || info == nullptr)
        return false;
    ...
    if (info->Instance == VK_NULL_HANDLE || info->PhysicalDevice == VK_NULL_HANDLE ||
        info->Device == VK_NULL_HANDLE || info->Queue == VK_NULL_HANDLE ||
        info->MinImageCount < 2 || info->ImageCount < info->MinImageCount)
        return false;
    if ((info->DescriptorPool != VK_NULL_HANDLE) == (info->DescriptorPoolSize != 0))
        return false;
    if (info->UseDynamicRendering && info->PipelineInfoMain.RenderPass != VK_NULL_HANDLE)
        return false;
...
void ImGui_ImplVulkan_Shutdown(bool destroy_device_objects)
{
    ImGui_ImplVulkan_Data* bd = ImGui_ImplVulkan_GetBackendData();
    if (bd == nullptr)
        return;
    ...
    else
    {
        for (ImTextureData* tex : platform_io.Textures)
        {
            if (tex->BackendUserData != nullptr)
                IM_DELETE((ImGui_ImplVulkan_Texture*)tex->BackendUserData);
            tex->BackendUserData = nullptr;
            tex->SetTexID(ImTextureID_Invalid);
            tex->SetStatus(ImTextureStatus_Destroyed);
        }
...
{
    ImGui_ImplVulkan_Data* bd = ImGui_ImplVulkan_GetBackendData();
    if (bd == nullptr || min_image_count < 2)
        return;
    if (bd->VulkanInitInfo.MinImageCount == min_image_count)
        return;

    if (!bd->MainWindowRenderBuffers.FrameRenderBuffers.empty())
    {
        check_vk_result(VK_NOT_READY);
        return;
    }
    bd->VulkanInitInfo.MinImageCount = min_image_count;
}

bool ImGui_ImplVulkan_SetImageCount(uint32_t image_count)
{
    ImGui_ImplVulkan_Data* bd = ImGui_ImplVulkan_GetBackendData();
    if (bd == nullptr || image_count < bd->VulkanInitInfo.MinImageCount)
        return false;
    if (bd->VulkanInitInfo.ImageCount == image_count)
        return true;
    ImGui_ImplVulkan_InitInfo* v = &bd->VulkanInitInfo;
    ImGui_ImplVulkan_DestroyWindowRenderBuffers(v->Device, &bd->MainWindowRenderBuffers, v->Allocator);
    v->ImageCount = image_count;
    return true;
}

bool ImGui_ImplVulkan_SetImageCounts(uint32_t min_image_count, uint32_t image_count)
{
    ImGui_ImplVulkan_Data* bd = ImGui_ImplVulkan_GetBackendData();
    if (bd == nullptr || min_image_count < 2 || image_count < min_image_count)
        return false;
    ImGui_ImplVulkan_InitInfo* v = &bd->VulkanInitInfo;
    if (v->MinImageCount == min_image_count && v->ImageCount == image_count)
        return true;
    ImGui_ImplVulkan_DestroyWindowRenderBuffers(v->Device, &bd->MainWindowRenderBuffers, v->Allocator);
    v->MinImageCount = min_image_count;
    v->ImageCount = image_count;
    return true;
}
...
{
    ImGui_ImplVulkan_Data* bd = ImGui_ImplVulkan_GetBackendData();
    if (bd == nullptr || sampler == VK_NULL_HANDLE || image_view == VK_NULL_HANDLE)
        return VK_NULL_HANDLE;
    ImGui_ImplVulkan_InitInfo* v = &bd->VulkanInitInfo;
...
{
    VkSurfaceFormatKHR invalid_format = { VK_FORMAT_UNDEFINED, (VkColorSpaceKHR)0 };
    if (!g_FunctionsLoaded || request_formats == nullptr || request_formats_count <= 0)
    ...
    VkResult err = vkGetPhysicalDeviceSurfaceFormatsKHR(physical_device, surface, &avail_count, nullptr);
    check_vk_result(err);
    if (err != VK_SUCCESS || avail_count == 0)
        return invalid_format;
    ImVector<VkSurfaceFormatKHR> avail_format;
    avail_format.resize((int)avail_count);
    err = vkGetPhysicalDeviceSurfaceFormatsKHR(physical_device, surface, &avail_count, avail_format.Data);
    check_vk_result(err);
    if (err != VK_SUCCESS || avail_count == 0)
        return invalid_format;
...
{
    if (!g_FunctionsLoaded || request_modes == nullptr || request_modes_count <= 0)
        return VK_PRESENT_MODE_MAX_ENUM_KHR;

    // Request a certain mode and confirm that it is available. If not use VK_PRESENT_MODE_FIFO_KHR which is mandatory
    uint32_t avail_count = 0;
    VkResult err = vkGetPhysicalDeviceSurfacePresentModesKHR(physical_device, surface, &avail_count, nullptr);
    check_vk_result(err);
    if (err != VK_SUCCESS || avail_count == 0)
        return VK_PRESENT_MODE_MAX_ENUM_KHR;
    ImVector<VkPresentModeKHR> avail_modes;
    avail_modes.resize((int)avail_count);
    err = vkGetPhysicalDeviceSurfacePresentModesKHR(physical_device, surface, &avail_count, avail_modes.Data);
    check_vk_result(err);
    if (err != VK_SUCCESS || avail_count == 0)
        return VK_PRESENT_MODE_MAX_ENUM_KHR;
    //for (uint32_t avail_i = 0; avail_i < avail_count; avail_i++)
...
bool ImGui_ImplVulkanH_CreateWindowCommandBuffers(VkPhysicalDevice physical_device, VkDevice device, ImGui_ImplVulkanH_Window* wd, uint32_t queue_family, const VkAllocationCallbacks* allocator)
{
    if (physical_device == VK_NULL_HANDLE || device == VK_NULL_HANDLE || wd == nullptr)
        return false;
...
static void ImGui_ImplVulkanH_DestroyWindowSwapchainData(VkDevice device, ImGui_ImplVulkanH_Window* wd, const VkAllocationCallbacks* allocator)
{
    for (uint32_t i = 0; i < wd->ImageCount; i++)
...
bool ImGui_ImplVulkanH_WaitForQueue(VkDevice device, VkQueue queue, ImGui_ImplVulkanH_Window* wd, const VkAllocationCallbacks* allocator, uint64_t wait_timeout_ns)
{
    if (device == VK_NULL_HANDLE || queue == VK_NULL_HANDLE || wd == nullptr || wait_timeout_ns == 0)
        return false;

    VkResult err;
    if (wd->QueueCompletionFence == VK_NULL_HANDLE)
    {
        VkFenceCreateInfo fence_info = {};
        fence_info.sType = VK_STRUCTURE_TYPE_FENCE_CREATE_INFO;
        fence_info.flags = VK_FENCE_CREATE_SIGNALED_BIT;
        err = vkCreateFence(device, &fence_info, allocator, &wd->QueueCompletionFence);
        check_vk_result(err);
        if (err != VK_SUCCESS)
            return false;
    }

    err = vkWaitForFences(device, 1, &wd->QueueCompletionFence, VK_TRUE, wait_timeout_ns);
    check_vk_result(err);
    if (err != VK_SUCCESS)
        return false;
    err = vkResetFences(device, 1, &wd->QueueCompletionFence);
    check_vk_result(err);
    if (err != VK_SUCCESS)
        return false;

    err = vkQueueSubmit(queue, 0, nullptr, wd->QueueCompletionFence);
    check_vk_result(err);
    if (err != VK_SUCCESS)
    {
        vkDestroyFence(device, wd->QueueCompletionFence, allocator);
        wd->QueueCompletionFence = VK_NULL_HANDLE;
        return false;
    }
    err = vkWaitForFences(device, 1, &wd->QueueCompletionFence, VK_TRUE, wait_timeout_ns);
    check_vk_result(err);
...
bool ImGui_ImplVulkanH_CreateWindowSwapChain(VkPhysicalDevice physical_device, VkDevice device, VkQueue queue, ImGui_ImplVulkanH_Window* wd, const VkAllocationCallbacks* allocator, int w, int h, uint32_t min_image_count, VkImageUsageFlags image_usage, uint64_t wait_timeout_ns)
{
    if (physical_device == VK_NULL_HANDLE || device == VK_NULL_HANDLE || queue == VK_NULL_HANDLE ||
        wd == nullptr || wd->Surface == VK_NULL_HANDLE || w <= 0 || h <= 0)
        return false;

    if (wd->Swapchain != VK_NULL_HANDLE && !ImGui_ImplVulkanH_WaitForQueue(device, queue, wd, allocator, wait_timeout_ns))
        return false;
    ...
    auto fail = [&]()
    {
        ImGui_ImplVulkanH_DestroyWindowSwapchainData(device, wd, allocator);
    ...
    VkResult err = vkGetPhysicalDeviceSurfaceCapabilitiesKHR(physical_device, wd->Surface, &cap);
    check_vk_result(err);
    if (err != VK_SUCCESS)
        return fail();
    ...
    if ((cap.supportedUsageFlags & info.imageUsage) != info.imageUsage)
        return fail();
    ...
    {
        info.imageExtent.width = (uint32_t)w;
        info.imageExtent.height = (uint32_t)h;
    ...
    if (err != VK_SUCCESS || wd->ImageCount == 0)
        return fail();
    ImVector<VkImage> backbuffers;
    backbuffers.resize((int)wd->ImageCount);
    ...
    wd->Frames.resize((int)wd->ImageCount);
    wd->FrameSemaphores.resize((int)wd->SemaphoreCount);
    memset(wd->Frames.Data, 0, wd->Frames.size_in_bytes());
    memset(wd->FrameSemaphores.Data, 0, wd->FrameSemaphores.size_in_bytes());
    for (uint32_t i = 0; i < wd->ImageCount; i++)
        wd->Frames[i].Backbuffer = backbuffers[(int)i];
    ...
    if (wd->UseDynamicRendering == false)
    {
        ...
        err = vkCreateRenderPass(device, &render_pass_info, allocator, &wd->RenderPass);
        check_vk_result(err);
        if (err != VK_SUCCESS)
            return fail();
    }
    ...
    view_info.format = wd->SurfaceFormat.format;
    ...
    for (uint32_t i = 0; i < wd->ImageCount; i++)
    {
        ImGui_ImplVulkanH_Frame* fd = &wd->Frames[(int)i];
        view_info.image = fd->Backbuffer;
        err = vkCreateImageView(device, &view_info, allocator, &fd->BackbufferView);
        check_vk_result(err);
        if (err != VK_SUCCESS)
            return fail();
    }
    ...
    if (wd->UseDynamicRendering == false)
    {
        ...
        framebuffer_info.renderPass = wd->RenderPass;
        framebuffer_info.attachmentCount = 1;
        framebuffer_info.pAttachments = attachment;
        framebuffer_info.width = (uint32_t)wd->Width;
        framebuffer_info.height = (uint32_t)wd->Height;
        framebuffer_info.layers = 1;
        for (uint32_t i = 0; i < wd->ImageCount; i++)
        {
            ImGui_ImplVulkanH_Frame* fd = &wd->Frames[(int)i];
            attachment[0] = fd->BackbufferView;
            err = vkCreateFramebuffer(device, &framebuffer_info, allocator, &fd->Framebuffer);
            check_vk_result(err);
            if (err != VK_SUCCESS)
                return fail();
        }
...
bool ImGui_ImplVulkanH_CreateOrResizeWindow(VkInstance instance, VkPhysicalDevice physical_device, VkDevice device, VkQueue queue, ImGui_ImplVulkanH_Window* wd, uint32_t queue_family, const VkAllocationCallbacks* allocator, int width, int height, uint32_t min_image_count, VkImageUsageFlags image_usage, uint64_t wait_timeout_ns)
{
    if (!g_FunctionsLoaded || instance == VK_NULL_HANDLE || physical_device == VK_NULL_HANDLE ||
        device == VK_NULL_HANDLE || queue == VK_NULL_HANDLE || wd == nullptr)
        return false;
    if (!ImGui_ImplVulkanH_CreateWindowSwapChain(physical_device, device, queue, wd, allocator, width, height, min_image_count, image_usage, wait_timeout_ns))
        return false;
    if (!ImGui_ImplVulkanH_CreateWindowCommandBuffers(physical_device, device, wd, queue_family, allocator))
    {
        ImGui_ImplVulkanH_DestroyWindowSwapchainData(device, wd, allocator);
        return false;
    }
...
void ImGui_ImplVulkanH_DestroyWindow(VkInstance instance, VkDevice device, ImGui_ImplVulkanH_Window* wd, const VkAllocationCallbacks* allocator)
{
    ...
    if (instance != VK_NULL_HANDLE && wd->Surface != VK_NULL_HANDLE)
...
void ImGui_ImplVulkanH_DestroyFrameSemaphores(VkDevice device, ImGui_ImplVulkanH_FrameSemaphores* fsd, const VkAllocationCallbacks* allocator)
{
    if (fsd == nullptr)
        return;
    if (fsd->ImageAcquiredSemaphore != VK_NULL_HANDLE)
        vkDestroySemaphore(device, fsd->ImageAcquiredSemaphore, allocator);
    if (fsd->RenderCompleteSemaphore != VK_NULL_HANDLE)
