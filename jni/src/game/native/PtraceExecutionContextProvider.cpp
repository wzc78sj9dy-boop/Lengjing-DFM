...
int PtracePacgaOracleReader::ReadThreadPointer(
    std::int32_t threadId,
    std::uint64_t& tpidrEl0) {
    tpidrEl0 = 0;
    if (threadId <= 0) return -EINVAL;
    ...
    int waitStatus = 0;
    if (!WaitForThread(threadId, waitStatus, waitTimeout)) {
        return fail(errno != 0 ? errno : EIO);
    }
    if (WIFEXITED(waitStatus) || WIFSIGNALED(waitStatus)) {
        impl_->pending = {};
        return -ESRCH;
    }
    if (!WIFSTOPPED(waitStatus)) return fail(EIO);
