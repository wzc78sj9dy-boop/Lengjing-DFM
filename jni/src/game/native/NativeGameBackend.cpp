...
public:
    ...
    bool Open(const RuntimeOptions& options,
        ...
        algorithmExecutionContextReady_ =
            algorithmPositionConfig_.IsConfigured() &&
...
private:
    ...
    bool ReadCharacterPosition(
        ...
        if (native::ShouldAttemptAlgorithmPosition(
            ...
            if (memory_ != nullptr && algorithmPositionRuntime_.Execute(
                    *memory_,
                    moduleBase_,
                    algorithmPositionConfig_.decryptRva,
                    actor,
                    ...
                    candidate)) {
                const Vec3 decoded{candidate.x, candidate.y, candidate.z};
    ...
    bool ReadBoneFrame(const RuntimeActorRecord& actorRecord,
        ...
        const auto readBoneSource =
            [this, &mesh, &boneArray, &componentMatrix](
                ...
                const bool componentReady = rebuildResolvedTransform
                    ? RebuildResolvedComponentTransform(
                          root, candidate, componentTransform)
    ...
    void CollectTrackingAcquisitionCandidate(
        ...
        if (!rangeTargetClass) {
            boneFrameReady = ReadBoneFrame(
                record, context.view, antiFlicker, boneFrame);
            if (boneFrameReady &&
