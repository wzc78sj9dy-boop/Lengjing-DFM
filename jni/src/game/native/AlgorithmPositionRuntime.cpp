...
constexpr std::uint64_t kPointerPayloadMask = 0x00FFFFFFFFFFFFFFULL;
constexpr std::size_t kMaximumCachedPages = 4096;
constexpr std::size_t kRetainedCachedPages = 3072;
constexpr std::size_t kRefreshBatchSize = 1024;
...
struct AlgorithmPositionRuntime::Impl {
    ...
    bool Execute(MemoryTransport& memory,
        ...
        try {
            ...
            if (!memory.IsOpen() || !ResolveEntry(
                    moduleBase, decryptRva, entityAddress, entry)) {
                return false;
            }
            ...
            UpdateExecutionContextLocked(
                memory, moduleBase, decryptRva, tpidrEl0, pacgaKey);
            ...
            if (pending == pendingExecutions_.end()) {
                if (pendingExecutions_.size() >= kMaximumCachedPages) {
...
private:
    ...
    void ClearExecutionContextLocked() noexcept {
        ...
        executionContextModuleBase_ = 0;
        executionContextDecryptRva_ = 0;
    ...
    void UpdateExecutionContextLocked(
        ...
        const bool changed = executionContextMemory_ != &memory ||
            executionContextModuleBase_ != moduleBase ||
            executionContextDecryptRva_ != decryptRva ||
    ...
    void PruneCompletedLocked(
        ...
        while (!completedOrder_.empty()) {
            ...
            const bool overLimit = current &&
                completedExecutions_.size() >= kMaximumCachedPages;
    ...
    void WorkerMain() noexcept {
        while (true) {
            ...
            const bool succeeded = ExecuteSynchronously(
                *execution.memory,
                execution.moduleBase,
                execution.decryptRva,
    ...
    bool ExecuteSynchronously(
        ...
        try {
            ...
            if (!memory.IsOpen() || !ResolveEntry(
                    moduleBase, decryptRva, entityAddress, entry) ||
                !EnsureEngine(memory, moduleBase)) {
                return false;
            }
            if (!IsExecutionCurrent(generation)) return false;
            ...
            if (hasHistory &&
                ...
                AlgorithmPosition second{};
                const bool hasSecond = ExecuteOnce(
                    memory,
                    entry,
                    entityAddress,
                    tpidrEl0,
    ...
    bool ResolveEntry(std::uintptr_t moduleBase,
                      std::uintptr_t decryptRva,
                      std::uintptr_t entityAddress,
                      std::uint64_t& entry) const {
        if (!AlgorithmPositionRuntimeConfig{decryptRva}.IsConfigured() ||
            !IsRemoteAddress(moduleBase) ||
            decryptRva > std::numeric_limits<std::uintptr_t>::max() -
                moduleBase ||
            !IsRemoteAddress(entityAddress & kPointerPayloadMask)) {
            return false;
        }
        entry = moduleBase + decryptRva;
        return IsRemoteAddress(entry & kPointerPayloadMask);
    }
    ...
    bool LoadRemotePage(std::uint64_t faultAddress) {
        ...
        const std::uint64_t remotePage =
            (guestPage & kPointerPayloadMask) & kPageMask;
    ...
    bool TrimCachedPages() {
        if (cachedPages_.size() <= kMaximumCachedPages) return true;
        while (cachedPages_.size() > kRetainedCachedPages) {
    ...
    bool RefreshCachedPages(MemoryTransport& memory,
        ...
        data.reserve(kRefreshBatchSize * kPageSize);
        requests.reserve(kRefreshBatchSize);
        status.reserve(kRefreshBatchSize);
        for (std::size_t begin = 0; begin < cachedPages_.size();
             begin += kRefreshBatchSize) {
            if (!IsExecutionCurrent(generation)) return false;
            const std::size_t end = std::min(
                cachedPages_.size(), begin + kRefreshBatchSize);
            ...
            static_cast<void>(memory.ReadBatch(
                requests.data(), requests.size(), status.data()));
            for (std::size_t index = begin; index < end; ++index) {
                if (status[index - begin] == 0) continue;
                const CachedPage& page = cachedPages_[index];
                if (uc_mem_write(
                        engine_,
                        page.guestAddress,
    ...
    std::uintptr_t executionContextModuleBase_ = 0;
    std::uintptr_t executionContextDecryptRva_ = 0;
    ...
    std::uintptr_t activeModuleBase_ = 0;
