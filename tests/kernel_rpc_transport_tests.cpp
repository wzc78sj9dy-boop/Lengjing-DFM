...
void TestMetadataOperations() {
    KernelRpcTransport transport(&FakeSyscall);
    const auto defaultCapabilities = transport.Capabilities();
    CHECK(!defaultCapabilities.attachProcessProtocolKnown);
    CHECK(defaultCapabilities.processExecutionContextRead);
    CHECK(!defaultCapabilities.threadScopedExecutionContext);
    CHECK(defaultCapabilities.installApgaKey);
