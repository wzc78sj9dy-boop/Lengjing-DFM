...
void RunAuthTransportTests() {
    ...
    const auto httpsStart = steady_clock::now();
    const T3HttpTransportResult httpsResult = transport->post(
        "https://127.0.0.1/private", "text/plain", "sensitive");
    const auto httpsElapsed = duration_cast<milliseconds>(
        steady_clock::now() - httpsStart);
    REQUIRE(!httpsResult.success);
    REQUIRE(httpsResult.error.find("HTTPS transport is unavailable") !=
            std::string::npos);
    REQUIRE(httpsElapsed < 100ms);
...

    {
        ...
        REQUIRE(result.error.find("HTTP receive failed") != std::string::npos);
