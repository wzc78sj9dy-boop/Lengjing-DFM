...
void TestStableRecordTableUsesLatestThreadRecords() {
    ...
    constexpr std::uintptr_t kSourceBaseOne =
        UINT64_C(0x6030310000);
    constexpr std::uintptr_t kMeshOne =
        UINT64_C(0x6030330000);
    ...
    InstallTable(
        ...
        &coordinateRecords,
        &ids);
    const std::vector<std::uint8_t> noiseRecords =
        MakeCoordinateRecords(
            static_cast<std::size_t>(kCount),
            0,
            static_cast<std::size_t>(kCount),
            10000.0f);
    transport.PutBytes(
        kOldNoiseBase, noiseRecords.data(), noiseRecords.size());
    transport.PutValue(
        kSourceBaseOne + kMeshCoordinateOffset,
        HardwareBreakpointCoordinate{100.0f, 200.0f, 30.0f});
    transport.PutValue(kMeshOne + kMeshIdOffset, kId);
    ...
    REQUIRE(transport.ReadCount(
                kOldNoiseBase, sizeof(std::uint64_t)) ==
            0);
    REQUIRE(transport.ReadCount(
    ...
    for (std::size_t index = 1; index < 70; ++index) {
        REQUIRE(runtime.Poll(
            kWorld, kTargetRoot, kIgnoredManager));
        REQUIRE(runtime.RecordsBase() == kRecordsBase);
        REQUIRE(runtime.Lookup(kId, kWorld, coordinate));
    }
...
void TestStableRecordTableUsesHighestHitPerThread() {
...
void TestStableRecordTableUsesLastEqualHit() {
    constexpr std::uintptr_t kBreakpoint = 0x41000;
    constexpr std::uintptr_t kWorld = 0x42000;
    constexpr std::uintptr_t kManager =
        UINT64_C(0x6050100000);
    constexpr std::uintptr_t kRecordsBase =
        UINT64_C(0x6050200000);
    constexpr std::uintptr_t kNoiseBase =
        UINT64_C(0x6050300000);
    constexpr std::uintptr_t kIdArray =
        UINT64_C(0x6050400000);
    constexpr std::int32_t kCount = 16;
    constexpr std::uint32_t kId = 8401;
    ...
    InstallTable(
        ...
        &ids);
    transport.PutValue(
        kNoiseBase, UINT64_C(0x1122334455667788));
    transport.AddBatch(
        {
            Record(7, kBreakpoint, kNoiseBase, 900),
            Record(7, kBreakpoint, kRecordsBase, 900),
        },
        kBreakpoint);
    ...
    REQUIRE(runtime.Start(
        ...
        transport.Callbacks(),
        HardwareBreakpointCoordinateProfile::StableRecordTable));
    REQUIRE(runtime.Poll(kWorld, kWorld, 0));
    REQUIRE(runtime.AcceptedSampleCount() == 1);
    REQUIRE(runtime.RecordsBase() == kRecordsBase);
    REQUIRE(transport.ReadCount(
                kNoiseBase, sizeof(std::uint64_t)) == 0);
    REQUIRE(transport.ReadCount(
...
void TestStableRecordTableUsesTenSlotMajority() {
    constexpr std::uintptr_t kBreakpoint = 0x43000;
    constexpr std::uintptr_t kWorld = 0x44000;
    constexpr std::uintptr_t kManager =
        UINT64_C(0x6060100000);
    constexpr std::uintptr_t kRecordsBaseOne =
        UINT64_C(0x6060200000);
    constexpr std::uintptr_t kRecordsBaseTwo =
        UINT64_C(0x6060300000);
    constexpr std::uintptr_t kIdArray =
        UINT64_C(0x6060400000);
    constexpr std::int32_t kCount = 16;
    constexpr std::uint32_t kId = 8501;

    const auto recordsOne = MakeCoordinateRecords(
        static_cast<std::size_t>(kCount),
        0,
        static_cast<std::size_t>(kCount),
        100.0f);
    const auto recordsTwo = MakeCoordinateRecords(
        static_cast<std::size_t>(kCount),
        0,
        static_cast<std::size_t>(kCount),
        1000.0f);
    ...
    transport.PutBytes(
        ...
        recordsTwo.data(),
        recordsTwo.size());
    for (std::uint64_t hit = 1; hit <= 5; ++hit) {
        transport.AddBatch(
            {Record(hit, kBreakpoint, kRecordsBaseOne, 901)},
            kBreakpoint);
    }
    for (std::uint64_t hit = 6; hit <= 11; ++hit) {
        transport.AddBatch(
            {Record(hit, kBreakpoint, kRecordsBaseTwo, 901)},
            kBreakpoint);
    }
    ...
    REQUIRE(runtime.Start(
        ...
        transport.Callbacks(),
        HardwareBreakpointCoordinateProfile::StableRecordTable));
    for (std::size_t poll = 0; poll < 10; ++poll) {
        REQUIRE(runtime.Poll(kWorld, kWorld, 0));
    }
    REQUIRE(runtime.RecordsBase() == kRecordsBaseOne);
    ...
    REQUIRE(runtime.Poll(kWorld, kWorld, 0));
    REQUIRE(runtime.RecordsBase() == kRecordsBaseTwo);
...
void RunHardwareBreakpointCoordinateRuntimeTests() {
    TestStableRecordTableUsesLatestThreadRecords();
    TestStableRecordTableUsesHighestHitPerThread();
    TestStableRecordTableUsesLastEqualHit();
    TestStableRecordTableUsesTenSlotMajority();
